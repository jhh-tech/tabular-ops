...
Desktop console for managing deployed Power BI / Analysis Services tabular models. Covers the operational workflows SSMS handles poorly: refresh orchestration, partition management, backup, and live trace viewing.

![Tabular Ops screenshot](docs/screenshot.png)
...
| 5 — Live trace | ⏳ Next | Event streaming via `Channel<T>`, trace list, filter chips |
| 6 — GUI alignment | ✅ Done | Fonts, light theme fix, startup perf, Overview tab, process model |
| 7 — Backup | 🚧 In progress | TOM `Database.Backup()`, ABF file picker, SQLite backup history |
| 8 — Power BI Enhanced Refresh | Planned | Cancel support, REST API refresh path |
...
| JetBrains Mono | [jetbrains.com/lp/mono](https://www.jetbrains.com/lp/mono/) — grab `JetBrainsMono-Regular.ttf`, `JetBrainsMono-SemiBold.ttf`, `JetBrainsMono-Bold.ttf` |
...
│   │   ├── Refresh/                  # TomRefreshEngine, BackupService, BackupStore, RefreshHistoryStore
│   │   ├── Tracing/                  # TraceCollector, TraceEvent, TraceEventChannel
│   │   └── Dmv/                      # DmvQueries, PartitionCacheStore
...
|---|---|---|
| TOM write | `Microsoft.AnalysisServices.Tabular.Server` | `RequestRefresh`, `SaveChanges`, schema reads, backup |
| ADOMD poll | `AdomdConnection` | Background DMV queries (sessions, memory) — active tenant only |
| Trace | `Microsoft.AnalysisServices.Tabular.Server` | Owned by `TraceCollector`; never used for commands |
...
**SQLite** (ADR-004): one DB file per data type in `%LocalAppData%\TabularOps\`. WAL mode is mandatory. Three stores:
- `refresh-history.db` — partition-level refresh runs, Enhanced Refresh workspace sync
- `partition-cache.db` — DMV stats cache for instant model switching (Phase 0 load)
- `backup-history.db` — model backup runs with file path and size
...
- **Power BI XMLA requires Premium/PPU/Fabric capacity** for read-write. Read-only XMLA works on any workspace but disables refresh and backup actions (shown in the status bar).
- **Backup on Power BI XMLA** requires the Contribute permission or higher on the workspace.
